...
export class AttackStatusEffectService {
  ...
  async applyStatusEffects(
    ...
    const hasStructuredStatusEffects =
      attack.hasEffects() &&
    ...
    if (!statusApplied && !hasStructuredStatusEffects) {
      ...
      if (parsedStatus) {
        ...
        if (requiresHeads || requiresTails) {
          ...
          const hasHeads = attackCoinFlipResults.some((result) =>
            result.isHeads(),
          );
          const hasTails = attackCoinFlipResults.some((result) =>
            result.isTails(),
          );
